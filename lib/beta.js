...
function each(dataSource, operation) {
  ...
  return prevState => {
    const items = (0, _.asData)(dataSource, prevState);
    ...
    }, prevState); // Ensure that the data this reducer was passed is returned to it's
    // original state. But allow any other changes to be kept.

    if (nextState.then) {
      return nextState.then(nextState => ({ ...nextState,
        data: prevState.data
      }));
    } else {
      return { ...nextState,
