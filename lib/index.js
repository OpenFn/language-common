import { unzip, zipObject, mapValues } from 'lodash';
 ...
 * @returns {<State>}
 */
export function map(path, operation) {
  return state => {
    switch (typeof path) {
      case 'string':
...
export function each(dataSource, operation) {
  ...
  console.warn(`
    This version of 'each' has been deprecated, future versions
    will ensure the data is returned to it's original state afterwards.

    To ensure your code is ready for the new implementation, please use
    'beta.each'.
  `)
